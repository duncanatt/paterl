...
type_seq([{type, _, pid, _Vars = []} | TypeSeq]) ->
  % Erlang PID types are not translated and eaten instead.
  type_seq(TypeSeq);
...
case_clauses(Clauses, Mb) ->
  ?TRACE("Translating case/receive clauses."),
...
case_clause(_Clause = {clause, _, PatSeq = [_], _GuardSeq = [], Body}, Mb) ->
  ...
  ?TRACE("Translating case/receive clause."),
%%  Mb0 = new_mb(Mb),
  Mb0 = fresh_mb(),
  Expr = expr_seq(Body, Mb0),
  pat_syntax:receive_expr(pat_seq(PatSeq), pat_syntax:var(Mb0), Expr).
...
expr_seq([Expr | ExprSeq], Mb) ->
  ?TRACE("About to translate expression ~p~n", [Expr]),
  {Expr0, Rest} = expr(Expr, ExprSeq, Mb),
  ?TRACE("After translating expression"),
%%  ExprSeq0 = expr_seq(Rest, fresh_mb()),
...
expr(Expr = {call, Anno, {atom, _, Name}, Args}, ExprSeq, Mb) ->
  ...
  % function call.
  ?TRACE("Translating a call expression ~p~n", [Expr]),
  Call0 =
    case paterl_anno:interface(Anno) of
      ...
      _Interface ->
        ...
        ?TRACE("Found modality: ~p", [paterl_anno:modality(Anno)]),
        case paterl_anno:modality(Anno) of
          new ->
            ?TRACE("Call expression with 'new' modality."),
            % Inject new mailbox.
            {Call, []} = expr(Expr, []),
            pat_syntax:tuple([Call, pat_syntax:var(Mb)]);

          use ->
            ?TRACE("Call expression with 'use' modality."),
...
expr({match, _, Pat, Expr}, ExprSeq, Mb) ->
  ...
  {Expr0, []} = expr(Expr, [], Mb),
  Mb1 = fresh_mb(),
  Binders = pat_syntax:tuple([pat(Pat), pat_syntax:var(Mb1)]),
...
expr({'receive', Anno, Clauses}, ExprSeq, Mb) ->
  ?TRACE("Translating receive expression."),
  ...
  ?TRACE("Obtained receive clauses ~p", [ReceiveClauses]),
...
fun_clause({clause, Anno, PatSeq, _GuardSeq = [], Body}) ->
  ...
  case paterl_anno:interface(Anno) of
    ...
    _Interface ->
      ...
      ?TRACE("About to translate expression sequence."),
