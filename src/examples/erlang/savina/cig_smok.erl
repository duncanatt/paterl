...
-use({smoker_mb, [smoker_loop/1, smoker_exit/0]}).
...
arbiter_loop(Num_rounds, Smoker1, Smoker2, Smoker3) ->
  ?mb_assert_regex("*Started_smoking"),
  receive
    {started_smoking} ->
...
%% @doc Smoker process main loop handling start smoking requests and issuing
%% started smoking replies to/from the arbiter.
-spec smoker_loop(arbiter_mb()) -> no_return().
smoker_loop(ArbiterMb) ->
  ?mb_assert_regex("*Start_smoking . *Exit"),
  receive
    {start_smoking, Ms} ->
...
%% ./src/paterl src/examples/erlang/savina/cig_smok.erl -v all -I include